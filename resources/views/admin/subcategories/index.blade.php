...
@section('title','SubCategories View')
...
<style>
    ...
    #subcategoriesTable { max-height: 400px; overflow-y: auto; display: block; }
    #subcategoriesTable th, #subcategoriesTable td { text-align: center; padding: 8px; }
</style>
    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col grid-margin">
                <div class="card">
                    <div class="card-body">
                        <h1>{{ __('messages.subcategories') }}</h1>
                        <a href="{{ route('subcategories.create', ['lang' => app()->getLocale()]) }}" class="btn btn-primary create">
                            <i class="fas fa-plus-circle"></i> {{ __('messages.create_subcategory') }}
                        </a>
                        <table id="subcategoriesTable" class="display">
                            <thead>
                                <tr>
                            ...
                            @foreach ($subcategories as $subcategory)
                                <tr> 
                                    <td>{{ $subcategory->{'name_' . $lang} }}</td>
                                    <td>{!! $subcategory->{'desc_' . $lang}  !!}</td>
                                    <td>{{ $subcategory->category->{'name_' . $lang} }}</td>
                                    <td>
                                        <a href="{{ route('subcategories.edit', ['lang' => app()->getLocale(), 'subcategory' => $subcategory->id]) }}" class="btn btn-outline-warning btn-sm">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form id="delete-form-{{ $subcategory->id }}" action="{{ route('subcategories.destroy', ['subcategory' => $subcategory->id, 'lang' => app()->getLocale()]) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-outline-danger btn-sm" onclick="confirmDelete({{ $subcategory->id }})">
