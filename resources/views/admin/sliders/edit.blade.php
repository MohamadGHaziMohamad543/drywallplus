        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('sliders.update', ['lang' => app()->getLocale(), 'slider' => $slider->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT') <!-- Use POST for update -->
                        @foreach (['en', 'ar'] as $lang)
                            <div class="form-group">
                                <label for="title_{{ $lang }}">{{ __('messages.name') }} ({{ strtoupper($lang) }}):</label>
                                <input type="text" class="form-control" name="title_{{ $lang }}" id="title_{{ $lang }}" value="{{ old('title_' . $lang, $slider->{'title_' . $lang}) }}">
                                @error('title_' . $lang)
                        ...
                        @foreach (['en', 'ar'] as $lang)
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description_{{ $lang }}" id="desc_{{ $lang }}">{!! old('description_' . $lang, $slider->{'description_' . $lang}) !!}</textarea>
                        ...
                        <div class="mb-3">
                                ...
                                <div id="drop-area-image" class="drop-area">
                                    ...
                                    <img id="image-preview" src="{{ asset($slider->image_path) }}" alt="Image Preview" style="margin-top: 10px; max-width: 100%;">
                                </div>
                            </div>
                                 @error('image')
                        ...
                        <button type="submit" class="btn btn-primary mr-2">
                            <i class="fas fa-save"></i> {{ __('messages.update_slider') }}
                        </button>
                    </form>
                </div>
            </div>
